...
def test_check(core_impl, input_cls) -> None:  # type:ignore
    ...
    assert not check(input_cls([3, object()]))
...
def test_apply_fuzz(core_impl, input_cls) -> None:  # type:ignore
    ...
    for _ in range(FUZZ_TEST_COUNT):
        ...
        random_ot_raw_list = normalize(
            input_cls(utils.make_random_ots(doc, FUZZ_TEST_OTS_LENGTH))
        )
...
def test_normalize(core_impl, input_cls) -> None:  # type:ignore
    ...
    assert normalize(input_cls([3, 4, "as", "df", {"d": "qw"}, {"d": "er"}, 5])) == [
        7,
        "asdf",
        {"d": "qwer"},
    ]
...
def test_transform_fuzz(core_impl, input_cls) -> None:  # type:ignore
    ...
    for _ in range(FUZZ_TEST_COUNT):
        ...
        random_ot_raw_list_1 = normalize(
            input_cls(utils.make_random_ots(doc, FUZZ_TEST_OTS_LENGTH))
        )

        random_ot_raw_list_2 = normalize(
            input_cls(utils.make_random_ots(doc, FUZZ_TEST_OTS_LENGTH))
        )
...
def test_compose_fuzz(core_impl, input_cls) -> None:  # type:ignore
    ...
    for _ in range(FUZZ_TEST_COUNT):
        ...
        random_ot_raw_list_1 = normalize(
            input_cls(utils.make_random_ots(doc_1, FUZZ_TEST_OTS_LENGTH))
        )

        doc_2 = apply(doc_1, random_ot_raw_list_1)
        random_ot_raw_list_2 = normalize(
            input_cls(utils.make_random_ots(doc_2, FUZZ_TEST_OTS_LENGTH))
        )

        doc_3 = apply(doc_2, random_ot_raw_list_2)

        doc_3_composed = apply(
            doc_1, compose(random_ot_raw_list_1, random_ot_raw_list_2)
        )
...
def test_complex_fuzz(core_impl, input_cls) -> None:  # type:ignore
    ...
    for _ in range(FUZZ_TEST_COUNT):
        ...
        #            while the editor holds `local_ot_raw_list_1`
        #            and `local_ot_raw_list_1a` in a buffer  that is not sended
        #            to the server. How does the editor transform the buffer?

        doc = utils.make_random_doc(FUZZ_TEST_INIT_DOC_LENGTH)
        local_ot_raw_list_1 = normalize(
            input_cls(utils.make_random_ots(doc, FUZZ_TEST_OTS_LENGTH))
        )
        local_ot_raw_list_1a = normalize(
            input_cls(
                utils.make_random_ots(
                    apply(doc, local_ot_raw_list_1), FUZZ_TEST_OTS_LENGTH
                )
            )
        )

        server_ot_raw_list_2 = normalize(
            input_cls(utils.make_random_ots(doc, FUZZ_TEST_OTS_LENGTH))
        )

        left_first_doc = apply(
            apply(apply(doc, local_ot_raw_list_1), local_ot_raw_list_1a),
            transform(
                server_ot_raw_list_2,
                compose(local_ot_raw_list_1, local_ot_raw_list_1a),
                "left",
            ),
        )
